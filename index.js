...
function calculateTotal(cart,req){
     total = 0;
     for(let i=0; i < cart.length; i++){
          //offering discounted price
          if(cart[i].sale_price){
               total = total + ( cart[i].sale_price * cart[i].quentity);
          }
          else{
               total = total + (cart[i].price * cart[i].quentity);
          }
     }
...
app.post('/add_to_cart', function(req,res) {
     ...
     var product = {id:id,name:name,price:price,sale_price:sale_price,quentity:quentity,images:images};
...
app.post('/edit_product_quentity', function(req,res) {
     //get values from input
     var id = req.body.id;
     var  quentity = req.body.quentity;
     var increase_btn = req.body.increase_product_quentity;
     var decrease_btn = req.body.decrease_product_quentity;
          ...
          for(let i=0; i<cart.length; i++){
               if(cart[i].id == id){
                    if(cart[i].quentity > 0){
                         cart[i].quentity = parseInt(cart[i].quentity) + 1;
                    }
               }
          }
     }
...
app.get('/cheakout',function(req,res) {
     var total = req.session.total;
     res.render('pages/cheakout', {total:total});
})
     ...
     con.connect((err)=>{
          if(err){
               ...
               con.query(query,[values],(err,result)=> {
                    ...
                    for(let i=0; i<cart.length; i++){
                         var query = "INSERT INTO order_items (order_id,product_id,product_name,product_price,product_image,product_quentity,order_date)";
                         var values = [
                              [id,cart[i].id,cart[i].name,cart[i].price,cart[i].images,cart[i].quentity,new Date()]
                         ];
                         con.query(query,[values],(err,result)=>{})
                    }
